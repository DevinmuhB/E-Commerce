...
if ($produkLangsung) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_checkout'])) {
        ...
        // Validasi alamat
        if (empty($alamat)) {
            echo "<script>alert('Alamat pengiriman harus diisi!'); window.history.back();</script>";
            exit;
        }

        if ($produkLangsung) {
            $stmt = $conn->prepare("INSERT INTO pesanan (id_user, id_produk, nama_penerima, email, telepon, alamat, latitude, longitude, status)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Menunggu Pembayaran')");
            $stmt->bind_param("iissssss", $user_id, $id_beli, $nama, $email, $telepon, $alamat, $latitude, $longitude);
            $stmt->execute();
            ...
            foreach ($produkList as $item) {
                $stmt = $conn->prepare("INSERT INTO pesanan (id_user, id_produk, nama_penerima, email, telepon, alamat, latitude, longitude, status)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Menunggu Pembayaran')");
                $stmt->bind_param("iissssss", $user_id, $item['id_produk'], $nama, $email, $telepon, $alamat, $latitude, $longitude);
                $stmt->execute();
                $pesanan_ids[] = $conn->insert_id;
            }
            $conn->prepare("DELETE FROM keranjang WHERE id_user = ?")->execute([$user_id]);
            $pesanan_id = $pesanan_ids[0]; // Ambil ID pesanan pertama untuk redirect
        }

        echo "<script>alert('Pesanan berhasil dibuat! Silakan lakukan pembayaran.'); window.location='payment.php?id=" . $pesanan_id . "';</script>";
        exit;
    }
            ...
            <?php else: ?>
                <?php foreach ($produkList as $produk): ?>
                    <?php
                        ...
                        $pathFoto = 'admin/' . ($produk['foto'] ?? 'uploads/default.png');
                    ?>
                    <div class="produk-item">
                        <img src="<?= htmlspecialchars($pathFoto) ?>" alt="<?= htmlspecialchars($produk['nama_produk']) ?>">
                        <div>
