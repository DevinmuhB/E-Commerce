...
<head>
    ...
    <style>
        .detail-wrap { max-width:1100px; margin:40px auto; background:#fff; border-radius:16px; box-shadow:0 4px 24px rgba(0,0,0,0.08); display:flex; gap:40px; padding:36px 32px; }
        .detail-imgs { flex:0 0 340px; }
        .main-img { width:100%; height:320px; object-fit:cover; border-radius:12px; border:1px solid #eee; }
        .thumbs { display:flex; gap:10px; margin-top:12px; }
        .thumbs img { width:60px; height:60px; object-fit:cover; border-radius:8px; border:2px solid #eee; cursor:pointer; transition:0.2s; }
        .thumbs img.active, .thumbs img:hover { border-color:#800000; }
        .detail-info { flex:1; }
        .prod-title { font-size:2em; color:#800000; margin-bottom:8px; }
        .prod-price { font-size:1.5em; color:#d32f2f; font-weight:700; margin-bottom:12px; }
        .prod-desc { color:#444; font-size:1.1em; margin-bottom:18px; }
        .prod-rating { margin-bottom:10px; }
        .prod-rating .fa-star { color:#FFD700; }
        .prod-rating .fa-star.empty { color:#ddd; }
        .btns { display:flex; gap:16px; margin-bottom:24px; }
        .btn-beli, .btn-keranjang { padding:12px 32px; border:none; border-radius:6px; font-size:1.1em; font-weight:600; cursor:pointer; }
        .btn-beli { background:#800000; color:#fff; }
        .btn-beli:hover { background:#a83232; }
        .btn-keranjang { background:#fff; color:#800000; border:2px solid #800000; }
        .btn-keranjang:hover { background:#f7f7fa; }
        .ulasan-section, .serupa-section {
            max-width: 800px;
        ...
        .ulasan-item {
            background:#f7f7fa; border-radius:10px; padding:18px 20px; margin-bottom:18px; box-shadow:0 2px 8px rgba(0,0,0,0.03);
        }
        .ulasan-user { font-weight:600; color:#800000; }
        .ulasan-rating .fa-star { color:#FFD700; }
        .ulasan-rating .fa-star.empty { color:#ddd; }
        .ulasan-text { margin:8px 0 0 0; color:#333; }
        .ulasan-img { margin-top:8px; max-width:120px; border-radius:8px; border:1px solid #eee; }
        .serupa-list {
            display:flex; gap:18px; flex-wrap:wrap; justify-content:center; align-items:center;
        }
        .serupa-item { width:170px; background:#fff; border-radius:10px; box-shadow:0 2px 8px rgba(0,0,0,0.04); padding:12px; text-align:center; }
        .serupa-item img { width:100%; height:110px; object-fit:cover; border-radius:8px; border:1px solid #eee; }
        .serupa-item .name { font-size:1em; color:#800000; margin:8px 0 4px 0; }
        .serupa-item .price { color:#d32f2f; font-weight:600; }
        .serupa-item a { text-decoration:none; color:inherit; }
    </style>
    <script>
    ...
    <div class="items_in_cart">
        ...
        ?>
            <div class="cart_item" style="display:flex; gap:10px;">
                <img src="admin/<?= htmlspecialchars($item['path_foto']) ?>" width="50">
                <div>
...
<div class="detail-wrap">
    <div class="detail-imgs">
        <img class="main-img" src="admin/<?= htmlspecialchars($fotos[0]) ?>" alt="<?= htmlspecialchars($produk['nama_produk']) ?>">
        <div class="thumbs">
            <?php foreach ($fotos as $i => $foto): ?>
                <img src="admin/<?= htmlspecialchars($foto) ?>" class="<?= $i === 0 ? 'active' : '' ?>" alt="thumb">
            <?php endforeach; ?>
        </div>
    </div>
            ...
            <?php foreach ($produk_serupa as $serupa): ?>
                <div class="serupa-item">
                    <a href="detail_produk.php?id=<?= $serupa['id_produk'] ?>">
                        <img src="admin/<?= htmlspecialchars($serupa['path_foto'] ?? 'uploads/default.png') ?>" alt="<?= htmlspecialchars($serupa['nama_produk']) ?>">
